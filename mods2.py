...
def main():
    ...
    next_golden_time = time.time()
    next_lifesaver_time = time.time() + 6
    ...
    class Worm(Falling_Object):
        ...
        def __init__(self, level):
            ...
            self.apples = 0 #inserted variable for cost
            self.lives = 1 
    ...
    while repeat_game:      # This outer loop is to play again on a win/loss
        ...
        while not stop_game:
            ...
            if time.time() > next_golden_time:
                golden = Golden_Apple()
                next_golden_time = time.time() + 7
            if time.time() > next_lifesaver_time:
                lifesaver = Life_Saver()
                next_lifesaver_time = next_lifesaver_time + random.randint(2, 4)
            ...
            if player_loss == False and player_victory == False:
                for catchable in all_catchables:
                    if pygame.sprite.collide_rect(player, catchable):
                        ...
                        apples_caught += catchable.apples
                        lives_remaining += catchable.lives
                for avoidable in all_avoidables:
                    if pygame.sprite.collide_rect(player, avoidable):
                        avoidable.kill()
                        avoidable.rect.top = height + 100
                        lives_remaining -= avoidable.lives  #allows us to customize how much each will cost
